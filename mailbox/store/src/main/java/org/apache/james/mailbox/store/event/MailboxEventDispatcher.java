...
public class MailboxEventDispatcher {
    ...
    public void added(MailboxSession session, SortedMap<MessageUid, MessageMetaData> uids, Mailbox mailbox, Map<MessageUid, MailboxMessage> cachedMessages) {
        listener.event(eventFactory.added(Optional.of(session.getSessionId()), session.getUser().getCoreUser(), uids, mailbox, cachedMessages));
    }
    ...
    public void expunged(MailboxSession session,  Map<MessageUid, MessageMetaData> uids, Mailbox mailbox) {
        if (!uids.isEmpty()) {
            listener.event(eventFactory.expunged(Optional.of(session.getSessionId()), session.getUser().getCoreUser(), uids, mailbox));
        }
    }
    ...
    public void flagsUpdated(MailboxSession session, List<MessageUid> uids, Mailbox mailbox, List<UpdatedFlags> uflags) {
        if (!uids.isEmpty()) {
            listener.event(eventFactory.flagsUpdated(Optional.of(session.getSessionId()), session.getUser().getCoreUser(), uids, mailbox, uflags));
        }
    }
    ...
    public void mailboxRenamed(MailboxSession session, MailboxPath from, Mailbox to) {
        listener.event(eventFactory.mailboxRenamed(Optional.of(session.getSessionId()), session.getUser().getCoreUser(), from, to));
    }
    ...
    public void mailboxDeleted(MailboxSession session, Mailbox mailbox, QuotaRoot quotaRoot, QuotaCount deletedMessageCount, QuotaSize totalDeletedSize) {
        listener.event(eventFactory.mailboxDeleted(Optional.of(session.getSessionId()), session.getUser().getCoreUser(), mailbox, quotaRoot, deletedMessageCount, totalDeletedSize));
    }
    ...
    public void mailboxAdded(MailboxSession session, Mailbox mailbox) {
        listener.event(eventFactory.mailboxAdded(Optional.of(session.getSessionId()), session.getUser().getCoreUser(), mailbox));
    }

    public void aclUpdated(MailboxSession session, MailboxPath mailboxPath, ACLDiff aclDiff, MailboxId mailboxId) {
        listener.event(eventFactory.aclUpdated(Optional.of(session.getSessionId()), session.getUser().getCoreUser(), mailboxPath, aclDiff, mailboxId));
    }

    public void moved(MailboxSession session, MessageMoves messageMoves, Map<MessageUid, MailboxMessage> messages) {
        MessageMoveEvent moveEvent = eventFactory.moved(Optional.of(session.getSessionId()), session.getUser().getCoreUser(), messageMoves, messages);
        if (!moveEvent.isNoop()) {
