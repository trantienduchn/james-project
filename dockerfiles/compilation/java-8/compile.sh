...
CASSANDRA_RABBITMQ_DESTINATION=/cassandra-rabbit/destination
...
if [ $? -eq 0 ]; then
   if [ -d "$CASSANDRA_RABBITMQ_DESTINATION" ]; then
      ...
      cp server/container/guice/cassandra-guice/target/james-server-cassandra-rabbitmq-guice.jar $CASSANDRA_RABBITMQ_DESTINATION || true
      cp -r server/container/guice/cassandra-guice/target/james-server-cassandra-rabbitmq-guice.lib $CASSANDRA_RABBITMQ_DESTINATION || true
      ...
      # cp server/container/guice/cassandra-ldap-guice/target/james-server-cassandra-rabbitmq-ldap-guice.jar $CASSANDRA_RABBITMQ_DESTINATION || true
      # cp -r server/container/guice/cassandra-ldap-guice/target/james-server-cassandra-ldap-rabbitmq-guice.lib $CASSANDRA_RABBITMQ_DESTINATION || true
   fi
