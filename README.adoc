 ...
 ** https://github.com/apache/james-project/#generating-a-package-for-james-with-cassandra--rabbitmq[Generating a package for James With Cassandra + RabbitMQ]
 ** https://github.com/apache/james-project/#generating-a-package-for-james-with-cassandra[Generating a package for James With Cassandra]
...
=== Generating a package for James With Cassandra + RabbitMQ
...
Note: A helper script is provided for the generation of packages for a specific git commit. For instance:
    ...
    $ sh dockerfiles/packaging/guice/cassandra-rabbitmq/package.sh 3.2.0 1 c298195e84 $PWD/result
...
**Packaging James LDAP with Cassandra + RabbitMQ is not supported yet.**

=== Generating a package for James With Cassandra
You can generate a deb package and an RPM package for James by using the following process.
