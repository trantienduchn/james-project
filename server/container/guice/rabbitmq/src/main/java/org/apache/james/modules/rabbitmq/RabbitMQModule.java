...
public class RabbitMQModule extends AbstractModule {
    ...
    private RabbitMQConfiguration getMailQueueConfiguration(PropertiesProvider propertiesProvider) throws ConfigurationException {
        Configuration configuration = loadConfigurationFiles(propertiesProvider);
        return RabbitMQConfiguration.from(configuration);
    }

    @Provides
    @Singleton
    private CassandraMailQueueViewConfiguration getMailQueueViewConfiguration(PropertiesProvider propertiesProvider) throws ConfigurationException {
        Configuration configuration = loadConfigurationFiles(propertiesProvider);
        return CassandraMailQueueViewConfiguration.from(configuration);
    }

    private Configuration loadConfigurationFiles(PropertiesProvider propertiesProvider) throws ConfigurationException {
        try {
